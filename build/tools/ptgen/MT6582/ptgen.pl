...
my $COMBO_NAND_KERNELH = "$CD_ALPS/kernel-3.4/arch/arm/mach-mt6582/$PROJECT/common/combo_nand.h";
...
my $COPY_PATH_KERNEL_PARTITION_H="$CD_ALPS/kernel-3.4/arch/arm/mach-mt6582/$PROJECT/common/";
my $COPY_PATH_PARTITION_DEFINE_H_NAME_PL="$CD_ALPS/bootable/bootloader/preloader/custom/$PROJECT/inc/";
my $COPY_PATH_PARTITION_DEFINE_H_NAME_LK="$CD_ALPS/bootable/bootloader/lk/target/$PROJECT/inc/";
my $COPY_PATH_PARTITION_DEFINE_H_NAME_K="$CD_ALPS/kernel-3.4/arch/arm/mach-mt6582/$PROJECT/common/";
my $COPY_PATH_PARTITION_DEFINE_C_NAME    = "$CD_ALPS/kernel-3.4/arch/arm/mach-mt6582/$PROJECT/common/";
my $COPY_PATH_PART_SIZE_LOCATION="$CD_ALPS/device/mediatek/$PROJECT/";
my $COPY_PATH_PMT_H_NAME_PL="$CD_ALPS/bootable/bootloader/preloader/custom/$PROJECT/inc/";
my $COPY_PATH_PMT_H_NAME_LK="$CD_ALPS/bootable/bootloader/lk/target/$PROJECT/inc/";
my $COPY_PATH_PMT_H_NAME_K="$CD_ALPS/kernel-3.4/arch/arm/mach-mt6582/$PROJECT/common/";
my $COPY_PATH_PreloaderC="$CD_ALPS/bootable/bootloader/preloader/custom/$PROJECT/inc/";
my $COPY_PATH_LK_MT_PartitionH="$CD_ALPS/bootable/bootloader/lk/target/$PROJECT/inc/";
my $COPY_PATH_LK_PartitionC="$CD_ALPS/bootable/bootloader/lk/target/$PROJECT/inc/";
...
my $Total_Size=512*1024*1024; #Hard Code 512MB for 4+2 project FIX ME!!!!!
	...
	if($MTK_NAND_UBIFS_SUPPORT eq "yes"){
		...
		if($COMBO_NAND_SUPPORT eq "yes") {
			`chmod 777 $COMBO_NAND_KERNELH` if (-e $COMBO_NAND_KERNELH);
 	       		open (COMBO_NAND_KERNELH, ">$COMBO_NAND_KERNELH") or &error_handler("CAN NOT open $COMBO_NAND_KERNELH\n", __FILE__, __LINE__) ;	
			printf COMBO_NAND_KERNELH ("#define COMBO_NAND_BLOCK_SIZE %d\n", $PEB);
			printf COMBO_NAND_KERNELH ("#define COMBO_NAND_PAGE_SIZE %d\n", $IOSIZE);
    			close COMBO_NAND_KERNELH;
		
			`chmod 777 $COMBO_NAND_TOOLH` if (-e $COMBO_NAND_TOOLH);
        		open (COMBO_NAND_TOOLH, ">$COMBO_NAND_TOOLH") or &error_handler("CAN NOT open $COMBO_NAND_TOOLH\n", __FILE__, __LINE__) ;	
			printf COMBO_NAND_TOOLH ("#define COMBO_NAND_BLOCK_SIZE %d\n", $PEB);
			printf COMBO_NAND_TOOLH ("#define COMBO_NAND_PAGE_SIZE %d\n", $IOSIZE);
    			close COMBO_NAND_TOOLH;
		}
	}
...
sub do_copy_files_old {
	...
	if (-e "$SCAT_NAME_DIR/MT6582_Android_scatter.txt") {
		if(-e "$COPY_SCATTER_BR_FILES_PATH/MT6582_Android_scatter.txt"){
			`chmod 777 $COPY_SCATTER_BR_FILES_PATH/MT6582_Android_scatter.txt`;	
		}						
		`cp $SCAT_NAME_DIR/MT6582_Android_scatter.txt $COPY_SCATTER_BR_FILES_PATH`;
		print AUTO_CHECK_OUT_FILES "$COPY_SCATTER_BR_FILES_PATH/MT6582_Android_scatter.txt\n";
	}
