...
my %mapping_table;
my %feature_table;
get_modem_file_mapping(\%mapping_table, \%feature_table, undef, $project_path, $type_of_load);
...
if ((exists $feature_table{"Android.mk"}) && ($feature_table{"Android.mk"} ne ""))
{
	my $WRITE_HANDLE;
	if (open($WRITE_HANDLE, ">$dest_folder/Android.mk"))
	{
		print $WRITE_HANDLE $feature_table{"Android.mk"};
		close($WRITE_HANDLE);
	}
	else
	{
