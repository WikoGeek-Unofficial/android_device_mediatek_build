...
MTK_CUSTOM_FOLDERS  := $(MTK_PROJECT_NAME)
DEVICE_FOLDER := $(strip $(wildcard \
        device/* \
        ../../../device/* \
    ))
MTK_CUSTOM_PATH := $(shell find $(DEVICE_FOLDER) -maxdepth 1 -name $(MTK_PROJECT_NAME))
MTK_PROJECT_CONFIGS := $(addsuffix /ProjectConfig.mk,$(MTK_CUSTOM_PATH))
$(foreach p,$(MTK_PROJECT_CONFIGS),$(eval include $p))

#$(warning MTK_CUSTOM_FOLDERS = $(MTK_CUSTOM_FOLDERS) , MTK_PROJECT_CONFIGS = $(MTK_PROJECT_CONFIGS), FULL_PROJECT = $(FULL_PROJECT))
ifeq ($(MTK_PROJECT_CONFIGS),)
$(warning PWD = $(shell pwd))
$(error Can not find ProjectConfig.mk , MTK_PROJECT_CONFIGS = $(MTK_PROJECT_CONFIGS))
endif

export_var :=
$(foreach p,$(MTK_PROJECT_CONFIGS),$(foreach f,$(strip $(shell cat $p | \
    grep -v -E "^\s*#" | sed 's/\s*=\s*.*//g')),$(eval export_var+=$f)))
